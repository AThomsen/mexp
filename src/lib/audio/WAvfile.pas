...
{   - Fixed bug with duration calculating                                     }
...
//			function ReadFromFile(const FileName: string): Boolean;   { Load header }
			function ReadFromStream(const FStr: TStream): Boolean;   { Load header }
...
function ReadWAV(const FStr: TStream; var WAVData: WAVRecord): Boolean;
begin
  try
    Result := true;
		{Read header }
		FStr.Position := 0;
		FStr.ReadBuffer(WAVData, 40);
    { Read number of samples if exists }
    if WAVData.DataHeader <> DATA_CHUNK then
		begin
			FStr.Position := WAVData.FormatSize + 28;
			FStr.ReadBuffer(WAVData.SampleNumber, 4);
		end
  except
    { Error }
    Result := false;
...
function TWAVfile.ReadFromStream(const FStr: TStream): Boolean;   { Load header }
var
	WAVData: WAVRecord;
begin
